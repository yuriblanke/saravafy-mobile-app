...
import {
  fetchAllPublicPontosForBrowse,
  fetchCollectionTitleById,
} from "./data/addToCollection";
import {
...
export default function AddToCollection() {
  ...
  const collectionQuery = useQuery({
    ...
    staleTime: 2 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
    queryFn: () => fetchCollectionTitleById(collectionId),
  ...
  const allPontosQuery = useQuery({
    ...
    staleTime: 3 * 60 * 1000,
    gcTime: 45 * 60 * 1000,
    queryFn: fetchAllPublicPontosForBrowse,
    placeholderData: (prev) => prev,
  });
